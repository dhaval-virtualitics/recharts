...
var Line = (function (_super) {
        ...
        _this.getStrokeDasharray = function (length, totalLength, lines) {
            ...
            for (var i = 0, sum = 0;; sum += lines[i], ++i) {
                if (sum + lines[i] > remainLength) {
                    remainLines = __spreadArray(__spreadArray([], lines.slice(0, i), true), [remainLength - sum], false);
                    break;
                }
            }
            var emptyLines = remainLines.length % 2 === 0 ? [0, restLength] : [restLength];
            return __spreadArray(__spreadArray(__spreadArray([], Line.repeat(lines, count), true), remainLines, true), emptyLines, true).map(function (line) { return "".concat(line, "px"); }).join(', ');
        };
        _this.id = (0, DataUtils_1.uniqueId)('recharts-line-');
    ...
    Line.repeat = function (lines, count) {
        var linesUnit = lines.length % 2 !== 0 ? __spreadArray(__spreadArray([], lines, true), [0], false) : lines;
        var result = [];
        for (var i = 0; i < count; ++i) {
            result = __spreadArray(__spreadArray([], result, true), linesUnit, true);
        }
        return result;
    };
    Line.prototype.renderErrorBar = function () {
        ...
        var errorBarItems = (0, ReactUtils_1.findAllByType)(children, ErrorBar_1.ErrorBar.displayName);
        if (!errorBarItems) {
            return null;
        }
    ...
    Line.renderDotItem = function (option, props) {
        ...
        if (react_1.default.isValidElement(option)) {
            ...
            var className = (0, classnames_1.default)('recharts-line-dot', option ? option.className : '');
    ...
    Line.prototype.renderDots = function (needClip, clipPathId) {
        ...
        var lineProps = (0, types_1.filterProps)(this.props);
        var customDotProps = (0, types_1.filterProps)(dot, true);
        var dots = points.map(function (entry, i) {
            var dotProps = __assign(__assign(__assign({ key: "dot-".concat(i), r: 3 }, lineProps), customDotProps), { value: entry.value, dataKey: dataKey, cx: entry.x, cy: entry.y, index: i, payload: entry.payload });
            return Line.renderDotItem(dot, dotProps);
        });
        var dotsProps = {
            clipPath: needClip ? "url(#clipPath-".concat(clipPathId, ")") : null,
        };
        return (react_1.default.createElement(Layer_1.Layer, __assign({ className: "recharts-line-dots", key: "dots" }, dotsProps, { role: "img" }), dots));
    };
    Line.prototype.renderCurveStatically = function (points, needClip, clipPathId, props) {
        ...
        var curveProps = __assign(__assign(__assign(__assign({}, (0, types_1.filterProps)(others, true)), { fill: 'none', className: 'recharts-line-curve', clipPath: needClip ? "url(#clipPath-".concat(clipPathId, ")") : null, points: points }), props), { type: type, layout: layout, connectNulls: connectNulls });
    ...
    Line.prototype.renderCurveWithAnimation = function (needClip, clipPathId) {
        ...
        return (react_1.default.createElement(react_smooth_1.default, { begin: animationBegin, duration: animationDuration, isActive: isAnimationActive, easing: animationEasing, from: { t: 0 }, to: { t: 1 }, key: "line-".concat(animationId), onAnimationEnd: this.handleAnimationEnd, onAnimationStart: this.handleAnimationStart }, function (_a) {
            var t = _a.t;
            if (prevPoints) {
                ...
                var stepData = points.map(function (entry, index) {
                    ...
                    if (animateNewValues) {
                        var interpolatorX = (0, DataUtils_1.interpolateNumber)(width * 2, entry.x);
                        var interpolatorY = (0, DataUtils_1.interpolateNumber)(height / 2, entry.y);
                        return __assign(__assign({}, entry), { x: interpolatorX(t), y: interpolatorY(t) });
                    }
            ...
            var interpolator = (0, DataUtils_1.interpolateNumber)(0, totalLength);
            var curLength = interpolator(t);
            var currentStrokeDasharray;
            if (strokeDasharray) {
                var lines = "".concat(strokeDasharray).split(/[,\s]+/gim).map(function (num) { return parseFloat(num); });
    ...
    Line.prototype.render = function () {
        ...
        var layerClass = (0, classnames_1.default)('recharts-line', className);
    ...
    Line.getComposedData = function (_a) {
        ...
        var points = displayedData.map(function (entry, index) {
            var value = (0, ChartUtils_1.getValueByDataKey)(entry, dataKey);
