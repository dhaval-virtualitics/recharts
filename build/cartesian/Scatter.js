...
var Scatter = (function (_super) {
    ...
    function Scatter() {
        ...
        _this.id = (0, DataUtils_1.uniqueId)('recharts-scatter-');
        return _this;
    }
    ...
    Scatter.prototype.renderSymbolsStatically = function (points) {
        ...
        return points.map(function (entry, i) {
            var props = __assign(__assign({ key: "symbol-".concat(i) }, baseProps), entry);
            return (react_1.default.createElement(Layer_1.Layer, __assign({ className: "recharts-scatter-symbol" }, (0, types_1.adaptEventsOfChild)(_this.props, entry, i), { key: "symbol-".concat(i), role: "img" }), Scatter.renderSymbolItem(activeIndex === i ? activeShape : shape, props)));
        });
    };
    Scatter.prototype.renderSymbolsWithAnimation = function () {
        ...
        return (react_1.default.createElement(react_smooth_1.default, { begin: animationBegin, duration: animationDuration, isActive: isAnimationActive, easing: animationEasing, from: { t: 0 }, to: { t: 1 }, key: "pie-".concat(animationId), onAnimationEnd: this.handleAnimationEnd, onAnimationStart: this.handleAnimationStart }, function (_a) {
            var t = _a.t;
            var stepData = points.map(function (entry, index) {
                var prev = prevPoints && prevPoints[index];
                if (prev) {
                    var interpolatorCx = (0, DataUtils_1.interpolateNumber)(prev.cx, entry.cx);
                    var interpolatorCy = (0, DataUtils_1.interpolateNumber)(prev.cy, entry.cy);
                    var interpolatorSize = (0, DataUtils_1.interpolateNumber)(prev.size, entry.size);
                    return __assign(__assign({}, entry), { cx: interpolatorCx(t), cy: interpolatorCy(t), size: interpolatorSize(t) });
                }
                var interpolator = (0, DataUtils_1.interpolateNumber)(0, entry.size);
    ...
    Scatter.prototype.renderLine = function () {
        ...
        var scatterProps = (0, types_1.filterProps)(this.props);
        var customLineProps = (0, types_1.filterProps)(line);
        var linePoints, lineItem;
        if (lineType === 'joint') {
            linePoints = points.map(function (entry) { return ({ x: entry.cx, y: entry.cy }); });
        }
        else if (lineType === 'fitting') {
            var _b = (0, DataUtils_1.getLinearRegression)(points), xmin = _b.xmin, xmax = _b.xmax, a_1 = _b.a, b_1 = _b.b;
    ...
    Scatter.prototype.render = function () {
        ...
        var layerClass = (0, classnames_1.default)('recharts-scatter', className);
        var needClip = (xAxis && xAxis.allowDataOverflow) || (yAxis && yAxis.allowDataOverflow);
        var clipPathId = lodash_1.default.isNil(id) ? this.id : id;
        return (react_1.default.createElement(Layer_1.Layer, { className: layerClass, clipPath: needClip ? "url(#clipPath-".concat(clipPathId, ")") : null },
    ...
    Scatter.getComposedData = function (_a) {
        ...
        var cells = (0, ReactUtils_1.findAllByType)(item.props.children, Cell_1.Cell.displayName);
        ...
        var points = displayedData.map(function (entry, index) {
            var x = (0, ChartUtils_1.getValueByDataKey)(entry, xAxisDataKey);
            var y = (0, ChartUtils_1.getValueByDataKey)(entry, yAxisDataKey);
            var z = (!lodash_1.default.isNil(zAxisDataKey) && (0, ChartUtils_1.getValueByDataKey)(entry, zAxisDataKey)) || '-';
            var tooltipPayload = [
                {
            ...
            var cx = (0, ChartUtils_1.getCateCoordinateOfLine)({
                axis: xAxis,
                ticks: xAxisTicks,
                bandSize: xBandSize,
                entry: entry,
            ...
            var cy = (0, ChartUtils_1.getCateCoordinateOfLine)({
                axis: yAxis,
                ticks: yAxisTicks,
                bandSize: yBandSize,
                entry: entry,
            ...
            return __assign(__assign(__assign({}, entry), { cx: cx, cy: cy, x: cx - radius, y: cy - radius, xAxis: xAxis, yAxis: yAxis, zAxis: zAxis, width: 2 * radius, height: 2 * radius, size: size, node: { x: x, y: y, z: z }, tooltipPayload: tooltipPayload, tooltipPosition: { x: cx, y: cy }, payload: entry }), (cells && cells[index] && cells[index].props));
        });
