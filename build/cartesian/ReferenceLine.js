...
var getEndPoints = function (scales, isFixedX, isFixedY, isSegment, props) {
    ...
    if (isFixedY) {
        ...
        if ((0, IfOverflowMatches_1.ifOverflowMatches)(props, 'discard') && !scales.y.isInRange(coord)) {
            return null;
        }
        var points = [
            { x: x + width, y: coord },
    ...
    if (isFixedX) {
        ...
        if ((0, IfOverflowMatches_1.ifOverflowMatches)(props, 'discard') && !scales.x.isInRange(coord)) {
            return null;
        }
        var points = [
            { x: coord, y: y + height },
    ...
    if (isSegment) {
        ...
        if ((0, IfOverflowMatches_1.ifOverflowMatches)(props, 'discard') && lodash_1.default.some(points, function (p) { return !scales.isInRange(p); })) {
            return null;
        }
        return points;
    }
...
function ReferenceLine(props) {
    ...
    (0, LogUtils_1.warn)(alwaysShow === undefined, 'The alwaysShow prop is deprecated. Please use ifOverflow="extendDomain" instead.');
    var scales = (0, CartesianUtils_1.createLabeledScales)({ x: xAxis.scale, y: yAxis.scale });
    var isX = (0, DataUtils_1.isNumOrStr)(fixedX);
    var isY = (0, DataUtils_1.isNumOrStr)(fixedY);
    ...
    var clipPath = (0, IfOverflowMatches_1.ifOverflowMatches)(props, 'hidden') ? "url(#".concat(clipPathId, ")") : undefined;
    var lineProps = __assign(__assign({ clipPath: clipPath }, (0, types_1.filterProps)(props, true)), { x1: x1, y1: y1, x2: x2, y2: y2 });
    return (react_1.default.createElement(Layer_1.Layer, { className: (0, classnames_1.default)('recharts-reference-line', className) },
        renderLine(shape, lineProps),
        Label_1.Label.renderCallByParent(props, (0, CartesianUtils_1.rectWithCoords)({ x1: x1, y1: y1, x2: x2, y2: y2 }))));
}
