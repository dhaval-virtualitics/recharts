...
var Bar = (function (_super) {
    ...
    function Bar() {
        ...
        _this.id = (0, DataUtils_1.uniqueId)('recharts-bar-');
    ...
    Bar.prototype.renderRectanglesStatically = function (data) {
        ...
        var baseProps = (0, types_1.filterProps)(this.props);
        return (data &&
            data.map(function (entry, i) {
                var props = __assign(__assign(__assign({}, baseProps), entry), { index: i });
                return (react_1.default.createElement(Layer_1.Layer, __assign({ className: "recharts-bar-rectangle" }, (0, types_1.adaptEventsOfChild)(_this.props, entry, i), { key: "rectangle-".concat(i), role: "img" }), Bar.renderRectangle(shape, props)));
            }));
    };
    Bar.prototype.renderRectanglesWithAnimation = function () {
        ...
        return (react_1.default.createElement(react_smooth_1.default, { begin: animationBegin, duration: animationDuration, isActive: isAnimationActive, easing: animationEasing, from: { t: 0 }, to: { t: 1 }, key: "bar-".concat(animationId), onAnimationEnd: this.handleAnimationEnd, onAnimationStart: this.handleAnimationStart }, function (_a) {
            var t = _a.t;
            var stepData = data.map(function (entry, index) {
                ...
                if (prev) {
                    ...
                    var interpolatorWidth = (0, DataUtils_1.interpolateNumber)(prev.width, entry.width);
                    var interpolatorHeight = (0, DataUtils_1.interpolateNumber)(prev.height, entry.height);
                ...
                if (layout === 'horizontal') {
                    var interpolatorHeight = (0, DataUtils_1.interpolateNumber)(0, entry.height);
                    var h = interpolatorHeight(t);
                    return __assign(__assign({}, entry), { y: entry.y + entry.height - h, height: h });
                }
                var interpolator = (0, DataUtils_1.interpolateNumber)(0, entry.width);
    ...
    Bar.prototype.renderBackground = function () {
        ...
        var backgroundProps = (0, types_1.filterProps)(this.props.background);
        return data.map(function (entry, i) {
            var value = entry.value, background = entry.background, rest = __rest(entry, ["value", "background"]);
            if (!background) {
                return null;
            }
            var props = __assign(__assign(__assign(__assign(__assign(__assign({}, rest), { fill: '#eee' }), background), backgroundProps), (0, types_1.adaptEventsOfChild)(_this.props, entry, i)), { index: i, key: "background-bar-".concat(i), className: 'recharts-bar-background-rectangle' });
    ...
    Bar.prototype.render = function () {
        ...
        var layerClass = (0, classnames_1.default)('recharts-bar', className);
        ...
        return (react_1.default.createElement(Layer_1.Layer, { className: layerClass },
            ...
            react_1.default.createElement(Layer_1.Layer, { className: "recharts-bar-rectangles", clipPath: needClip ? "url(#clipPath-".concat(clipPathId, ")") : null },
    ...
    Bar.getComposedData = function (_a) {
        ...
        var pos = (0, ChartUtils_1.findPositionOfBar)(barPosition, item);
        if (!pos) {
            return null;
        }
        ...
        var baseValue = (0, ChartUtils_1.getBaseValueOfBar)({ numericAxis: numericAxis });
        ...
        var rects = displayedData.map(function (entry, index) {
            ...
            if (layout === 'horizontal') {
                ...
                if (Math.abs(minPointSize) > 0 && Math.abs(height) < Math.abs(minPointSize)) {
                    var delta = (0, DataUtils_1.mathSign)(height || minPointSize) * (Math.abs(minPointSize) - Math.abs(height));
                    y -= delta;
                    height += delta;
                }
            }
                ...
                if (Math.abs(minPointSize) > 0 && Math.abs(width) < Math.abs(minPointSize)) {
                    var delta = (0, DataUtils_1.mathSign)(width || minPointSize) * (Math.abs(minPointSize) - Math.abs(width));
                    width += delta;
                }
            }
            return __assign(__assign(__assign(__assign({}, entry), { x: x, y: y, width: width, height: height, value: stackedData ? value : value[1], payload: entry, background: background }), (cells && cells[index] && cells[index].props)), { tooltipPayload: [(0, ChartUtils_1.getTooltipItem)(item, entry)], tooltipPosition: { x: x + width / 2, y: y + height / 2 } });
        });
