...
function LabelList(props) {
    ...
    return (react_1.default.createElement(Layer_1.Layer, { className: "recharts-label-list" }, data.map(function (entry, index) {
        var value = lodash_1.default.isNil(dataKey)
            ...
            : (0, ChartUtils_1.getValueByDataKey)(entry && entry.payload, dataKey);
        var idProps = lodash_1.default.isNil(id) ? {} : { id: "".concat(id, "-").concat(index) };
        return (react_1.default.createElement(Label_1.Label, __assign({}, (0, types_1.filterProps)(entry, true), others, idProps, { parentViewBox: entry.parentViewBox, index: index, value: value, textBreakAll: textBreakAll, viewBox: Label_1.Label.parseViewBox(lodash_1.default.isNil(clockWise) ? entry : __assign(__assign({}, entry), { clockWise: clockWise })), key: "label-".concat(index) })));
    })));
}
...
function renderCallByParent(parentProps, data, ckeckPropsLabel) {
    ...
    var explicitChilren = (0, ReactUtils_1.findAllByType)(children, LabelList.displayName).map(function (child, index) {
    ...
    return __spreadArray([implicitLabelList], explicitChilren, true);
}
