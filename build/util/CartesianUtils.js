...
var formatAxisMap = function (props, axisMap, offset, axisType, chartName) {
    ...
    var hasBar = !!(0, ReactUtils_1.findChildByType)(children, 'Bar');
    return ids.reduce(function (result, id) {
        ...
        if (axis.type === 'number' && (axis.padding === 'gap' || axis.padding === 'no-gap')) {
            ...
            var smallestDistanceBetweenValues_1 = Infinity;
            var sortedValues_1 = axis.categoricalDomain.sort();
            sortedValues_1.forEach(function (value, index) {
                if (index > 0) {
                    smallestDistanceBetweenValues_1 = Math.min((value || 0) - (sortedValues_1[index - 1] || 0), smallestDistanceBetweenValues_1);
                }
            });
            var smallestDistanceInPercent = smallestDistanceBetweenValues_1 / diff;
            var rangeWidth = axis.layout === 'vertical' ? offset.height : offset.width;
            if (axis.padding === 'gap') {
                calculatedPadding = (smallestDistanceInPercent * rangeWidth) / 2;
            }
            if (axis.padding === 'no-gap') {
                var gap = (0, DataUtils_1.getPercentValue)(props.barCategoryGap, smallestDistanceInPercent * rangeWidth);
        ...
        var _c = (0, ChartUtils_1.parseScale)(axis, chartName, hasBar), scale = _c.scale, realScaleType = _c.realScaleType;
        ...
        var finalAxis = __assign(__assign(__assign({}, axis), ticks), { realScaleType: realScaleType, x: x, y: y, scale: scale, width: axisType === 'xAxis' ? offset.width : axis.width, height: axisType === 'yAxis' ? offset.height : axis.height });
...
var rectWithCoords = function (_a) {
    ...
    return (0, exports.rectWithPoints)({ x: x1, y: y1 }, { x: x2, y: y2 });
};
...
var createLabeledScales = function (options) {
    ...
    return __assign(__assign({}, scales), { apply: function (coord, _a) {
            var _b = _a === void 0 ? {} : _a, bandAware = _b.bandAware, position = _b.position;
            return lodash_1.default.mapValues(coord, function (value, label) { return scales[label].apply(value, { bandAware: bandAware, position: position }); });
