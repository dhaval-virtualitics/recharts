...
function getDomainOfDataByKey(data, key, type, filterNil) {
    ...
    if (type === 'number') {
        var domain = flattenData.filter(function (entry) { return (0, DataUtils_1.isNumber)(entry) || parseFloat(entry); });
    ...
    return validateData.map(function (entry) { return ((0, DataUtils_1.isNumOrStr)(entry) || entry instanceof Date ? entry : ''); });
}
...
var getLegendProps = function (_a) {
    var children = _a.children, formattedGraphicalItems = _a.formattedGraphicalItems, legendWidth = _a.legendWidth, legendContent = _a.legendContent;
    ...
    if (legendItem.props && legendItem.props.payload) {
        ...
        legendData = (formattedGraphicalItems || []).reduce(function (result, _a) {
        ...
        legendData = (formattedGraphicalItems || []).map(function (_a) {
            var item = _a.item;
            ...
            return {
                ...
                color: (0, exports.getMainColorOfGraphicItem)(item),
                value: name || dataKey,
                payload: item.props,
            };
        });
...
var getBarSizeList = function (_a) {
    ...
    for (var i = 0, len = numericAxisIds.length; i < len; i++) {
        ...
        for (var j = 0, sLen = stackIds.length; j < sLen; j++) {
            ...
            var barItems = items.filter(function (item) { return (0, ReactUtils_1.getDisplayName)(item.type).indexOf('Bar') >= 0; });
...
var getBarPosition = function (_a) {
    ...
    if (len < 1)
        return null;
    var realBarGap = (0, DataUtils_1.getPercentValue)(barGap, bandSize, 0, true);
    var result;
    if (sizeList[0].barSize === +sizeList[0].barSize) {
        ...
        result = sizeList.reduce(function (res, entry) {
            var newRes = __spreadArray(__spreadArray([], res, true), [
                {
                    item: entry.item,
                    position: {
        ...
        var offset_1 = (0, DataUtils_1.getPercentValue)(barCategoryGap, bandSize, 0, true);
...
var appendOffsetOfLegend = function (offset, items, props, legendBox) {
    ...
    var legendProps = (0, exports.getLegendProps)({ children: children, legendWidth: legendWidth });
    var newOffset = offset;
    if (legendProps) {
        var box = legendBox || {};
        var align = legendProps.align, verticalAlign = legendProps.verticalAlign, layout = legendProps.layout;
        if ((layout === 'vertical' || (layout === 'horizontal' && verticalAlign === 'center')) && (0, DataUtils_1.isNumber)(offset[align])) {
            newOffset = __assign(__assign({}, offset), (_a = {}, _a[align] = newOffset[align] + (box.width || 0), _a));
        }
        if ((layout === 'horizontal' || (layout === 'vertical' && align === 'center')) && (0, DataUtils_1.isNumber)(offset[verticalAlign])) {
...
var isErrorBarRelevantForAxis = function (layout, axisType, direction) {
    if (lodash_1.default.isNil(axisType)) {
        return true;
    }
...
var getDomainOfErrorBars = function (data, item, dataKey, layout, axisType) {
    var children = item.props.children;
    var errorBars = (0, ReactUtils_1.findAllByType)(children, 'ErrorBar').filter(function (errorBarChild) {
...
var getTicksOfAxis = function (axis, isGrid, isAll) {
    ...
    offset = axis.axisType === 'angleAxis' ? (0, DataUtils_1.mathSign)(range[0] - range[1]) * 2 * offset : offset;
...
var parseScale = function (axis, chartType, hasBar) {
    ...
    if (lodash_1.default.isString(scale)) {
        var name_1 = "scale".concat(lodash_1.default.upperFirst(scale));
        return {
...
var truncateByDomain = function (value, domain) {
    ...
    if (!(0, DataUtils_1.isNumber)(value[0]) || value[0] < min) {
        result[0] = min;
    }
    if (!(0, DataUtils_1.isNumber)(value[1]) || value[1] > max) {
        result[1] = max;
    }
    if (result[0] > max) {
        result[0] = max;
...
var getTicksOfScale = function (scale, opts) {
    ...
    if (tickCount &&
        ...
        var tickValues = (0, recharts_scale_1.getNiceTickValues)(domain, tickCount, allowDecimals);
        scale.domain((0, exports.calculateDomainOfTicks)(tickValues, type));
        return { niceTicks: tickValues };
    }
    if (tickCount && type === 'number') {
        var domain = scale.domain();
        var tickValues = (0, recharts_scale_1.getTickValuesFixedDomain)(domain, tickCount, allowDecimals);
        return { niceTicks: tickValues };
    }
    return null;
};
...
var getCateCoordinateOfLine = function (_a) {
    ...
    if (axis.type === 'category') {
        if (!axis.allowDuplicatedCategory && axis.dataKey && !lodash_1.default.isNil(entry[axis.dataKey])) {
            var matchedTick = (0, DataUtils_1.findEntryInArray)(ticks, 'value', entry[axis.dataKey]);
            if (matchedTick) {
...
var parseSpecifiedDomain = function (specifiedDomain, dataDomain, allowDataOverflow) {
    if (lodash_1.default.isFunction(specifiedDomain)) {
