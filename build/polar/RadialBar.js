...
var RadialBar = (function (_super) {
    ...
    RadialBar.prototype.renderSectorsStatically = function (sectors) {
        ...
        var baseProps = (0, types_1.filterProps)(others);
        return sectors.map(function (entry, i) {
            var props = __assign(__assign(__assign(__assign(__assign({}, baseProps), { cornerRadius: cornerRadius }), entry), (0, types_1.adaptEventsOfChild)(_this.props, entry, i)), { key: "sector-".concat(i), className: 'recharts-radial-bar-sector', forceCornerRadius: others.forceCornerRadius, cornerIsExternal: others.cornerIsExternal });
    ...
    RadialBar.prototype.renderSectorsWithAnimation = function () {
        ...
        return (react_1.default.createElement(react_smooth_1.default, { begin: animationBegin, duration: animationDuration, isActive: isAnimationActive, easing: animationEasing, from: { t: 0 }, to: { t: 1 }, key: "radialBar-".concat(animationId), onAnimationStart: this.handleAnimationStart, onAnimationEnd: this.handleAnimationEnd }, function (_a) {
            var t = _a.t;
            var stepData = data.map(function (entry, index) {
                var prev = prevData && prevData[index];
                if (prev) {
                    var interpolatorStartAngle = (0, DataUtils_1.interpolateNumber)(prev.startAngle, entry.startAngle);
                    var interpolatorEndAngle = (0, DataUtils_1.interpolateNumber)(prev.endAngle, entry.endAngle);
                    return __assign(__assign({}, entry), { startAngle: interpolatorStartAngle(t), endAngle: interpolatorEndAngle(t) });
                }
                var endAngle = entry.endAngle, startAngle = entry.startAngle;
                var interpolator = (0, DataUtils_1.interpolateNumber)(startAngle, endAngle);
    ...
    RadialBar.prototype.renderBackground = function (sectors) {
        ...
        return sectors.map(function (entry, i) {
            ...
            var props = __assign(__assign(__assign(__assign(__assign(__assign({ cornerRadius: cornerRadius }, rest), { fill: '#eee' }), background), backgroundProps), (0, types_1.adaptEventsOfChild)(_this.props, entry, i)), { index: i, key: "sector-".concat(i), className: 'recharts-radial-bar-background-sector' });
        ...
        var sectors = displayedData.map(function (entry, index) {
            ...
            if (layout === 'radial') {
                ...
                if (Math.abs(minPointSize) > 0 && Math.abs(deltaAngle) < Math.abs(minPointSize)) {
                    var delta = (0, DataUtils_1.mathSign)(deltaAngle || minPointSize) * (Math.abs(minPointSize) - Math.abs(deltaAngle));
                    endAngle += delta;
                }
                backgroundSector = {
                    background: {
                ...
                if (Math.abs(minPointSize) > 0 && Math.abs(deltaRadius) < Math.abs(minPointSize)) {
                    var delta = (0, DataUtils_1.mathSign)(deltaRadius || minPointSize) * (Math.abs(minPointSize) - Math.abs(deltaRadius));
                    outerRadius += delta;
                }
            }
            return __assign(__assign(__assign(__assign(__assign({}, entry), backgroundSector), { payload: entry, value: stackedData ? value : value[1], cx: cx, cy: cy, innerRadius: innerRadius, outerRadius: outerRadius, startAngle: startAngle, endAngle: endAngle }), (cells && cells[index] && cells[index].props)), { tooltipPayload: [(0, ChartUtils_1.getTooltipItem)(item, entry)], tooltipPosition: (0, PolarUtils_1.polarToCartesian)(cx, cy, (innerRadius + outerRadius) / 2, (startAngle + endAngle) / 2) });
        });
