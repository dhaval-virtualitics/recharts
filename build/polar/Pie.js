...
var Pie = (function (_super) {
    ...
    function Pie(props) {
        ...
        _this.pieRef = null;
        _this.sectorRefs = [];
        _this.id = (0, DataUtils_1.uniqueId)('recharts-pie-');
    ...
    Pie.prototype.hasActiveIndex = function () {
        var activeIndex = this.props.activeIndex;
    ...
    Pie.prototype.renderLabels = function (sectors) {
        ...
        var pieProps = (0, types_1.filterProps)(this.props);
        var customLabelProps = (0, types_1.filterProps)(label);
        var customLabelLineProps = (0, types_1.filterProps)(labelLine);
        var offsetRadius = (label && label.offsetRadius) || 20;
        var labels = sectors.map(function (entry, i) {
            var midAngle = (entry.startAngle + entry.endAngle) / 2;
            var endPoint = (0, PolarUtils_1.polarToCartesian)(entry.cx, entry.cy, entry.outerRadius + offsetRadius, midAngle);
            ...
            var lineProps = __assign(__assign(__assign(__assign(__assign({}, pieProps), entry), { fill: 'none', stroke: entry.fill }), customLabelLineProps), { index: i, points: [(0, PolarUtils_1.polarToCartesian)(entry.cx, entry.cy, entry.outerRadius, midAngle), endPoint], key: 'line' });
            ...
            return (react_1.default.createElement(Layer_1.Layer, { key: "label-".concat(i) },
                labelLine && Pie.renderLabelLineItem(labelLine, lineProps),
                Pie.renderLabelItem(label, labelProps, (0, ChartUtils_1.getValueByDataKey)(entry, realDataKey))));
        });
    ...
    Pie.renderSectorItem = function (option, props) {
        ...
        if (lodash_1.default.isPlainObject(option)) {
            return react_1.default.createElement(Sector_1.Sector, __assign({ tabIndex: -1 }, props, option));
        }
        return react_1.default.createElement(Sector_1.Sector, __assign({ tabIndex: -1 }, props));
    };
    Pie.prototype.renderSectorsStatically = function (sectors) {
        var _this = this;
        var _a = this.props, activeShape = _a.activeShape, blendStroke = _a.blendStroke, inactiveShapeProp = _a.inactiveShape;
        return sectors.map(function (entry, i) {
            var inactiveShape = inactiveShapeProp && _this.hasActiveIndex() ? inactiveShapeProp : null;
            var sectorOptions = _this.isActiveIndex(i) ? activeShape : inactiveShape;
            var sectorProps = __assign(__assign({}, entry), { stroke: blendStroke ? entry.fill : entry.stroke });
            return (react_1.default.createElement(Layer_1.Layer, __assign({ ref: function (ref) {
                    if (ref && !_this.sectorRefs.includes(ref)) {
                        _this.sectorRefs.push(ref);
                    }
                }, tabIndex: -1, className: "recharts-pie-sector" }, (0, types_1.adaptEventsOfChild)(_this.props, entry, i), { key: "sector-".concat(i) }), Pie.renderSectorItem(sectorOptions, sectorProps)));
        });
    };
    Pie.prototype.renderSectorsWithAnimation = function () {
        ...
        return (react_1.default.createElement(react_smooth_1.default, { begin: animationBegin, duration: animationDuration, isActive: isAnimationActive, easing: animationEasing, from: { t: 0 }, to: { t: 1 }, key: "pie-".concat(animationId, "-").concat(prevIsAnimationActive), onAnimationStart: this.handleAnimationStart, onAnimationEnd: this.handleAnimationEnd }, function (_a) {
            var t = _a.t;
            var stepData = [];
            ...
            sectors.forEach(function (entry, index) {
                ...
                if (prev) {
                    var angleIp = (0, DataUtils_1.interpolateNumber)(prev.endAngle - prev.startAngle, entry.endAngle - entry.startAngle);
                    ...
                    var interpolatorAngle = (0, DataUtils_1.interpolateNumber)(0, endAngle - startAngle);
    ...
    Pie.prototype.attachKeyboardHandlers = function (pieRef) {
        var _this = this;
        pieRef.onkeydown = function (e) {
            if (!e.altKey) {
                switch (e.key) {
                    case 'ArrowLeft': {
                        var next = ++_this.state.sectorToFocus % _this.sectorRefs.length;
                        _this.sectorRefs[next].focus();
                        _this.setState({ sectorToFocus: next });
                        break;
                    }
                    case 'ArrowRight': {
                        var next = --_this.state.sectorToFocus < 0
                            ? _this.sectorRefs.length - 1
                            : _this.state.sectorToFocus % _this.sectorRefs.length;
                        _this.sectorRefs[next].focus();
                        _this.setState({ sectorToFocus: next });
                        break;
                    }
                    case 'Escape': {
                        _this.sectorRefs[_this.state.sectorToFocus].blur();
                        _this.setState({ sectorToFocus: 0 });
                        break;
                    }
                    default: {
                    }
    ...
    Pie.prototype.render = function () {
        ...
        var layerClass = (0, classnames_1.default)('recharts-pie', className);
        return (react_1.default.createElement(Layer_1.Layer, { tabIndex: 0, className: layerClass, ref: function (ref) {
                _this.pieRef = ref;
            } },
            this.renderSectors(),
    ...
    Pie.getRealPieData = function (item) {
        ...
        var presentationProps = (0, types_1.filterProps)(item.props);
        var cells = (0, ReactUtils_1.findAllByType)(children, Cell_1.Cell.displayName);
        if (data && data.length) {
    ...
    Pie.parseCoordinateOfPie = function (item, offset) {
        ...
        var maxPieRadius = (0, PolarUtils_1.getMaxRadius)(width, height);
        var cx = left + (0, DataUtils_1.getPercentValue)(item.props.cx, width, width / 2);
        var cy = top + (0, DataUtils_1.getPercentValue)(item.props.cy, height, height / 2);
        var innerRadius = (0, DataUtils_1.getPercentValue)(item.props.innerRadius, maxPieRadius, 0);
        var outerRadius = (0, DataUtils_1.getPercentValue)(item.props.outerRadius, maxPieRadius, maxPieRadius * 0.8);
    ...
    Pie.getComposedData = function (_a) {
        ...
        var realDataKey = dataKey;
        if (lodash_1.default.isNil(dataKey) && lodash_1.default.isNil(valueKey)) {
            (0, LogUtils_1.warn)(false, "Use \"dataKey\" to specify the value of pie,\n      the props \"valueKey\" will be deprecated in 1.1.0");
            realDataKey = 'value';
        }
        else if (lodash_1.default.isNil(dataKey)) {
            (0, LogUtils_1.warn)(false, "Use \"dataKey\" to specify the value of pie,\n      the props \"valueKey\" will be deprecated in 1.1.0");
        ...
        var notZeroItemCount = pieData.filter(function (entry) { return (0, ChartUtils_1.getValueByDataKey)(entry, realDataKey, 0) !== 0; }).length;
        var totalPadingAngle = (absDeltaAngle >= 360 ? notZeroItemCount : notZeroItemCount - 1) * paddingAngle;
        var realTotalAngle = absDeltaAngle - notZeroItemCount * minAngle - totalPadingAngle;
        var sum = pieData.reduce(function (result, entry) {
            var val = (0, ChartUtils_1.getValueByDataKey)(entry, realDataKey, 0);
        ...
        if (sum > 0) {
            ...
            sectors = pieData.map(function (entry, i) {
                var val = (0, ChartUtils_1.getValueByDataKey)(entry, realDataKey, 0);
                var name = (0, ChartUtils_1.getValueByDataKey)(entry, nameKey, i);
                var percent = ((0, DataUtils_1.isNumber)(val) ? val : 0) / sum;
                var tempStartAngle;
                if (i) {
                    tempStartAngle = prev_1.endAngle + (0, DataUtils_1.mathSign)(deltaAngle) * paddingAngle * (val !== 0 ? 1 : 0);
                }
                else {
                    tempStartAngle = startAngle;
                }
                var tempEndAngle = tempStartAngle + (0, DataUtils_1.mathSign)(deltaAngle) * ((val !== 0 ? minAngle : 0) + percent * realTotalAngle);
                ...
                var tooltipPosition = (0, PolarUtils_1.polarToCartesian)(coordinate.cx, coordinate.cy, middleRadius, midAngle);
                prev_1 = __assign(__assign(__assign({ percent: percent, cornerRadius: cornerRadius, name: name, tooltipPayload: tooltipPayload, midAngle: midAngle, middleRadius: middleRadius, tooltipPosition: tooltipPosition }, entry), coordinate), { value: (0, ChartUtils_1.getValueByDataKey)(entry, realDataKey), startAngle: tempStartAngle, endAngle: tempEndAngle, payload: entry, paddingAngle: (0, DataUtils_1.mathSign)(deltaAngle) * paddingAngle });
                return prev_1;
            });
        }
